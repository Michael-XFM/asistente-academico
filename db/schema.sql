...
-- Solo la definicion, vacia -- la carga masiva en si vive en
-- db/carga_masiva_log_actividad.sql, que NO corre en el bootstrap
-- (generar un millon de filas en cada arranque desde cero rompe la
-- reproducibilidad rapida del Bloque B). Sin trigger de fn_auditoria a
-- proposito: no es una de las 4 tablas auditadas -- si lo tuviera, la
-- carga masiva duplicaria su propio volumen en auditoria.
